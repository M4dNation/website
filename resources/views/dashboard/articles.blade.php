					...
					<div class="row">
						<div class="col-lg-12 col-md-12 col-sm-12 user-box">
							<p>
								...
								@if($article->state == 1)
									<a href= "{{ route('dashboard.draft.article', $article->number_label) }}" class="state label pull-right label-success">Published</a>
								@endif
								@if($article->state == 0)
									<a href= "{{ route('dashboard.publish.article', $article->number_label) }}" class="state label pull-right label-danger">Draft</a>
								@endif
							<p>
						</div>
					</div>
